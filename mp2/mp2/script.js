...
window.onload = function() {
    ...
    function readClassesFile() {
        ...
        varClass = JSON.parse(this.result);
        console.log(JSON.stringify(varClass, null, 2));
        localStorage.setItem("classes", JSON.stringify(varClass, null, 2));
        }
        classesReader.readAsText(this.files[0]);
        selectwhat = document.getElementById("selectwhat");
        selectwhat.innerHTML = "class upload saved to local storage . . .";
    };
}

function startRequest(){
    var classesRequest = new XMLHttpRequest();
    classesRequest.open('GET', 'http://192.168.1.2/classes.json');
    ...
    storedNotif = document.createElement("span");
    storedNotif.setAttribute("id", "storedNotif");
    storedTextNode = document.createTextNode("saved to local storage . . .");
    storedNotif.appendChild(storedTextNode);
    selectwhat = document.getElementById("selectwhat");
    selectwhat.innerHTML = "server class saved to local storage . . .";
...
function showClasscodes() {
    ...
    if(localStorage.getItem("classes") === null){
        selectwhat.innerHTML = "no classes . . .";
    }else
    {
    ...
    storedNotif = document.getElementById("storedNotif");
    if(storedNotif){
        box.removeChild(storedNotif);
    }
                            ...
                            if(lateDetails[4] == classes[x].classdays[y].classhours[z].classhour){
                                for(a=0; a<classes[x].classdays[y].classhours[z].students.length; a++){
                                    if(lateDetails[1] == classes[x].classdays[y].classhours[z].students[a].idno){
                                        ...
                                        localStorage.setItem("classes", JSON.stringify(classes, null, 2));
                                    }
                                }
                            }
                        }
...
function showReport(){
    ...
    xmlhttp.open('POST', 'http://192.168.1.15/classes.json');
    ...
    var data = localStorage.getItem("classes");
    xmlhttp.send(data);
}
